...
${test.err}`,
  ...
  function post(api, body, callback) {
    return axios.post(`${this.domain}?api/v2/${api}`, body, {
      headers: {
  ...
  this.upload = function upload(runs, name, description) {
    ...
    console.log(`Publishing ${testrailResults.length} test result(s) to ${this.domain}`);

    return post(`add_run/${this.projectId}`, {
      ...
      const runId = body.id;
      console.log(`Results published to ${this.domain}?/runs/view/${runId}`);
